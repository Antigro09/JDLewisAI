...
export async function runRollupAction(
  _prev: RollupState,
  formData: FormData,
): Promise<RollupState> {
  ...
  try {
    ...
    const sheet = await sheetsCreate(token, `Invoice Roll-Up — ${new Date().toLocaleDateString()}`, rows);
    ...
    return { error: err instanceof Error ? err.message : "Roll-up failed." };
  }
}
