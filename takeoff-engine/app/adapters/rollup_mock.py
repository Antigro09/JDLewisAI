"""Mock rollup — a deterministic builtin CSI/assembly table. In practice this
covers the MVP trades entirely; the LLM adapter only adds nicer descriptions
and coverage of unseen item types."""
        ...
        for item in items:
            ...
            if row is None:
                row = RollupRow(
                    ...
                    unit=item.get("unit", ""),
                    confidence=0.0,  # unmapped → surfaces as needing review upstream
                )
            out.append(row)
        return out
